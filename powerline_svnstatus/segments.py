...
class SvnStatusSegment(Segment):
    ...
    def execute_info(self, pl, segment_info):
        ...
        proc = Popen(
            ...
            stdout=PIPE,
            stderr=PIPE,
            env=self.make_env(segment_info),
        )
    ...
    def parse_info(self, lines, branch_re, line_start):
        if not line_start:
            line_start = self.default_line_start
        url = next((x for x in lines if x.startswith(line_start)), "")
        if not url:
            return ""

        if not branch_re:
            branch_re = self.default_branch_re
    ...
    def parse_status(self, lines):
        return Counter(
            [
                ...
                for x in lines
                if not re.match(
                    r"       |$|Performing|^Summary of conflicts|^  Text conflicts", x
                )
            ]
        )
    ...
    def __call__(
        self, pl, segment_info, branch_format=None, branch_re=None, line_start=None
    ):
        segments = []
